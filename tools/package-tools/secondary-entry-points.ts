 ...
 *
 * @param pkg The package for which to get entry points, e.g., 'ngx-groundhog'.
 * @returns An array of secondary entry-points names, e.g., ['button', 'tabs', ...]
 */
export function getSecondaryEntryPointsForPackage(pkg: BuildPackage) {
  ...
  // E.g., extract "portal" from "from '@dynatrace/ngx-groundhog/button';".
...
function buildPackageImportStatementFindCommand(searchDirectory: string, packageName: string) {
  if (platform() === 'win32') {
    return {
      binary: 'findstr',
      args: ['/r', `from.'@dynatrace/${packageName}/.*'`, `${searchDirectory}\\*.ts`]
    };
  } else {
    return {
      binary: 'grep',
      args: ['-Eroh', '--include', '*.ts', `from '@dynatrace/${packageName}/.+';`, searchDirectory]
    };
  }
}