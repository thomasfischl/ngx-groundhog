...
/** TODO: (ffr): figure out if we need rollup removeLicense at the beginning */
 ...
 */
  ...
  private async bundlePrimaryEntryPoint() {
    ...
    return this.bundleEntryPoint({
      ...
      moduleName: `ngx.${this.buildPackage.name}`,
  ...
  private async bundleSecondaryEntryPoint(entryPoint: string) {
    ...
    return this.bundleEntryPoint({
      entryFile,
      esm5EntryFile,
      moduleName: `ngx.${packageName}.${dashCaseToCamelCase(entryPoint)}`,
  ...
  private async createRollupBundle(config: RollupBundleConfig) {
    ...
    if (config.format === 'umd') {
      ...
      if (this.buildPackage.exportsSecondaryEntryPointsAtRoot &&
        ...
        // Use the rollup-alias plugin to map imports of the form `@dynatrace/ngx-groundhog/button`
  ...
  /**
   * Gets mapping of import aliases (e.g. `@dynatrace/ngx-groundhog/button`) to the path of the es5
   * bundle output.
  ...
  private getResolvedSecondaryEntryPointImportPaths(bundleOutputDir: string) {
    return this.buildPackage.secondaryEntryPoints.reduce((map, p) => {
      map[`@dynatrace/${this.buildPackage.name}/${p}`] =
